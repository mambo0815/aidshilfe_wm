...
class Bunyad_Customizer_Control_Typography extends WP_Customize_Control
{
	...
	public function to_json() 
	{
		...
		$this->json['font_sizes'] = array_combine(range(10, 35), range(10, 35));
	}

	public function enqueue()
	{
    	wp_enqueue_script('bunyad-customize-controls', get_template_directory_uri() . '/js/admin/customizer-controls.js', array('jquery'));
    	wp_enqueue_script('bunyad-customize-selectbox', get_template_directory_uri() . '/js/admin/selectize.js', array('jquery'));
    	wp_enqueue_style('bunyad-customize-selectbox', get_template_directory_uri() . '/css/admin/selectize.css');
	}
	...
	?>
		<label>	
			...
			<# if ( data.value.font_name ) { #>
			
			<select {{{ data.link }}} class="font_name">
				<# _.each( data.choices, function( label, choice ) { #>
					<option value="{{ choice }}"<# if ( choice === data.value.font_name ) { #>selected<# } #>>{{ label }}</option>
				<# } ) #>
			</select>
			
			<# } #>
			
			<# if ( data.value.font_weight ) { #>
			
			<div>
				<span class="sub-label"><?php echo esc_html_x('Weight:', 'Admin', 'smart-blog'); ?></span>
			...
			<# if ( data.value.font_size ) { #>
			
			<div>
				<span class="sub-label"><?php echo esc_html_x('Size:', 'Admin', 'smart-blog'); ?></span>
