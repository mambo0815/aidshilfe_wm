...
class Bunyad_Admin_OptionRenderer
{
	...
	public function _render_checkbox($element) {
		...
		$element['options'] = array_merge(
			array('checked' => esc_html_x('Yes', 'Admin', 'smart-blog'), 'unchecked' => esc_html_x('No', 'Admin', 'smart-blog')), 
	...
	{
		$button_label = esc_html_x( 'Upload', 'Admin', 'smart-blog' );
		...
		{ 
			...
			if (isset($element['bg_type'])) 
			{
				...
				$type .= $this->render_select(array(
					...
					'options' => array(
						'repeat' => esc_html_x('Repeat Horizontal and Vertical - Pattern', 'Admin', 'smart-blog'),
						'cover'  => esc_html_x('Fully Cover Background - Photo', 'Admin', 'smart-blog'),
						'repeat-x' => esc_html_x('Repeat Horizontal', 'Admin', 'smart-blog'),
						'repeat-y' => esc_html_x('Repeat Vertical', 'Admin', 'smart-blog'),
						'no-repeat' => esc_html_x('No Repeat', 'Admin', 'smart-blog'),
					),
				));
			}
			
			$output .= '<a href="" class="remove-image button after-upload'. $classes .'">'. esc_html_x('Remove', 'Admin', 'smart-blog') .'</a>';			
	...
	public function render_multiple($element)
	{
		...
		$output = '<div class="element-multiple">' . $fields . '<a href="#">' . esc_html_x('Add More', 'Admin', 'smart-blog')  . '</a></div>';
	...
	public function _render_multiple_fields($element, $key = null)
	{
		...
		return '<div class="fields'. ($key == null ? ' default' : '') .'">' . $fields . '<a href="#" class="remove">' . esc_html_x('Remove', 'Admin', 'smart-blog') . '</a></div>';
