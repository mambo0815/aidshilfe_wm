...
class Bunyad_Posts
{
	...
	public function excerpt($text = null, $length = 55, $options = array())
	{
		...
		// add defaults
		$options = array_merge(array('add_more' => null, 'force_more' => null, 'use_teaser' => false), $options);
	...
	public function paginate($options = array(), $query = null)
	{
		...
		$args = array(
			...
			'next_text' => '<span class="visuallyhidden">' . esc_html_x('Next', 'pagination', 'smart-blog') . '</span><i class="fa fa-angle-right"></i>',
			'prev_text' => '<i class="fa fa-angle-left"></i><span class="visuallyhidden">' . esc_html_x('Previous', 'pagination', 'smart-blog') . '</span>'
		);
	...
	public function excerpt_read_more()
	{
		...
		if (Bunyad::options()->read_more) {
			...
			$text = $this->more_text;
			if (!$text) {
				$text = esc_html__('Read More', 'smart-blog');
			}
